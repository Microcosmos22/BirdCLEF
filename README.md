...
In order to obtain the *features* (3rd image) we do use a simple image encoder `efficientnet_b0` implemented by the `timm` library, using pre-trained weights for the most part of the training. Finally, the features pass through an *attention* layer + *classifier*, which predict the animal class label. Eventually the top encoder layer will be unfrozen - this flexibility should allow the encoder to focus on features that correlate with the predicted class. 
